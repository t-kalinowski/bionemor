...
NVIDIA BioNeMo Recipes from R. Inputs and results are R vectors, data frames,
matrices, and durable job objects.
...
bionemo_doctor(compute, target = "all", verbose = FALSE)
```
...
matters at the lower level: `evo2()` can describe a model or custom checkpoint
offline, while `bionemo_compute()` describes the runtime, workspace, backend,
recipe revision, and GPU allocation. `evo2_model()` couples them for the usual
workflow.
