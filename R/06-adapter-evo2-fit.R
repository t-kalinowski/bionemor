...
fit_control_args <- function(control, model_record) {
  args <- c(
    c("--seq-length", as.character(control@sequence_length)),
    c("--global-batch-size", as.character(control@global_batch_size)),
    c("--micro-batch-size", as.character(control@micro_batch_size)),
    "--tensor-model-parallel-size",
    as.character(control@tensor_parallel_size),
    "--pipeline-model-parallel-size",
    as.character(control@pipeline_parallel_size),
    c("--context-parallel-size", as.character(control@context_parallel_size)),
    c("--mixed-precision-recipe", resolve_fit_precision(control, model_record)),
    c("--lr", format_number(control@learning_rate)),
    c("--min-lr", format_number(control@minimum_learning_rate)),
    c("--warmup-steps", as.character(control@warmup_steps)),
    c("--constant-steps", as.character(control@constant_steps)),
    c("--wd", format_number(control@weight_decay)),
    c("--eval-interval", as.character(control@eval_interval)),
    c("--eval-iters", as.character(control@eval_iters)),
    c("--log-interval", as.character(control@log_interval)),
    c("--clip-grad", format_number(control@clip_gradient)),
    c("--hidden-dropout", format_number(control@hidden_dropout)),
    c("--attention-dropout", format_number(control@attention_dropout)),
    c("--seed", as.character(control@seed)),
    c("--workers", as.character(control@workers)),
    c("--most-recent-k", as.character(control@keep_checkpoints))
  )
...
evo2_finetune <- function(
  ...
  run_path <- run$path
  args <- c(
    c("--nproc-per-node", as.character(compute@gpus)),
    "--no-python",
    "train_evo2",
    c("--dataset-config", data@manifest$dataset_config),
    c("--dataset-dir", data@path),
    c("--model-size", object@model_size),
    c("--hf-tokenizer-model-path", tokenizer),
    c("--max-steps", as.character(as.integer(steps))),
    c("--result-dir", output),
    c("--experiment-name", name),
    c("--finetune-ckpt-dir", resolved_checkpoint),
  ...
  if (S7_inherits(method, Evo2LoRA)) {
    args <- c(
      ...
      "--lora-finetune",
      c("--lora-dim", as.character(method@rank)),
      c("--lora-alpha", format_number(method@alpha)),
      c("--lora-dropout", format_number(method@dropout)),
      "--lora-target-modules",
