...
evo2_prepare <- function(
  ...
  tokenizer <- evo2_checkpoint_tokenizer(
    model@config$tokenizer %||% "recommended",
    record,
    compute
  )
  ...
  descriptor <- list(
    ...
    tokenizer = tokenizer,
    tokenizer_revision = record$tokenizer_revision,
    train = fasta$train %||% NULL,
    validation = fasta$validation %||% NULL,
    test = fasta$test %||% NULL,
    split = as.list(data@split),
    seed = data@seed,
    id_col = data@id_col,
    sequence_col = data@sequence_col,
    path = destination,
...
materialize_prepare_job <- function(job, descriptor) {
  ...
  Evo2Dataset(
    ...
    split = unlist(descriptor$split, use.names = TRUE),
    seed = as.integer(descriptor$seed),
    id_col = descriptor$id_col,
    sequence_col = descriptor$sequence_col,
    prepared = TRUE,
    path = descriptor$path,
    manifest = manifest,
    provenance = list(run_path = descriptor$run_path)
  )
}
...
evo2_finetune <- function(
  ...
  tokenizer <- evo2_checkpoint_tokenizer(
    object@config$tokenizer %||% "recommended",
    model_record,
    compute
  )
  ...
  descriptor <- list(
    ...
    base_checkpoint_source = checkpoint@source,
    base_checkpoint_source_trust = checkpoint_record$source_trust %||%
      "not-recorded",
    base_checkpoint_source_verified = isTRUE(
      checkpoint_record$source_verified
    ),
    family = "evo2",
    variant = object@size,
    model_size = object@model_size,
    tokenizer = tokenizer,
    tokenizer_revision = model_record$tokenizer_revision,
...
materialize_finetune_job <- function(job, descriptor) {
  if (!is.list(descriptor) || !identical(descriptor$type, "fine-tune")) {
    stop("fine-tuning result descriptor is invalid")
  }
  if (
  ...
  if (!identical(inspection$model_size, descriptor$model_size)) {
  ...
  base_digest <- if (is.null(descriptor$base_checkpoint)) {
    NULL
  } else {
    if (!dir.exists(descriptor$base_checkpoint)) {
  ...
  manifest <- list(
    ...
    format = "mbridge",
    kind = descriptor$checkpoint_kind,
    source = paste0("fit://", job@id),
    ...
    tokenizer = inspection$tokenizer %||% NULL,
    mixed_precision_recipe = descriptor$precision,
    ...
    provenance = list(
      ...
      gradient_accumulation = descriptor$gradient_accumulation,
  ...
  Evo2Model(
    ...
    model_size = descriptor$model_size,
    context_length = as.integer(descriptor$context_length),
