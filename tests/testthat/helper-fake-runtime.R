...
fake_recipes_runtime <- function(bin) {
  ...
  write_r_executable(
    ...
    c(
      ...
      "value <- function(flag) args[[match(flag, args) + 1L]]",
      "if (command == 'run') {",
      "  request_path <- normalizePath(value('--request'), mustWork = TRUE)",
      "  run_path <- dirname(request_path)",
      "  request <- jsonlite::read_json(request_path, simplifyVector = FALSE)",
      "  execution <- request$execution",
      "  stopifnot(execution$schema_version == 1L, execution$driver == 'evo2-megatron', execution$operation == 'generate')",
      "  resolve <- function(path) if (grepl('^/|^[A-Za-z]:[/\\\\]', path)) path else file.path(run_path, path)",
      "  parameters <- execution$parameters",
      "  resolved <- execution$resolved",
      "  torchrun_args <- c(",
      "    '--nproc-per-node', as.character(resolved$processes_per_node), '--no-python', 'infer_evo2',",
      "    '--ckpt-dir', execution$checkpoint,",
      "    '--prompt-file', resolve(execution$inputs$prompts),",
      "    '--max-new-tokens', as.character(parameters$max_new_tokens),",
      "    '--temperature', as.character(parameters$temperature),",
      "    '--top-k', as.character(parameters$top_k),",
      "    '--top-p', as.character(parameters$top_p),",
      "    '--output-file', resolve(execution$outputs$upstream),",
      "    '--tensor-parallel-size', as.character(resolved$tensor_parallel_size),",
      "    '--pipeline-model-parallel-size', as.character(resolved$pipeline_parallel_size),",
      "    '--context-parallel-size', as.character(resolved$context_parallel_size),",
      "    if (!is.null(resolved$mixed_precision_recipe)) c('--mixed-precision-recipe', resolved$mixed_precision_recipe),",
      "    if (isTRUE(resolved$vortex_style_fp8)) '--vortex-style-fp8',",
      "    if (!is.null(resolved$max_sequence_length)) c('--max-seq-length', as.character(resolved$max_sequence_length)),",
      "    '--max-batch-size', as.character(resolved$max_batch_size),",
      "    '--cuda-graph-impl', resolved$cuda_graphs,",
      "    if (isTRUE(resolved$subquadratic_ops)) '--use-subquadratic-ops',",
      "    if (isTRUE(resolved$chunked_prefill)) '--enable-chunked-prefill',",
      "    if (!is.null(resolved$dynamic_max_tokens)) c('--inference-dynamic-batching-max-tokens', as.character(resolved$dynamic_max_tokens)),",
      "    '--inference-dynamic-batching-block-size', as.character(resolved$dynamic_block_size),",
      "    if (!is.null(parameters$seed)) c('--seed', as.character(parameters$seed)),",
      "    if (isTRUE(parameters$return_probabilities)) '--return-log-probs'",
      "  )",
      "  status <- system2('torchrun', torchrun_args)",
      "  if (!identical(status, 0L)) quit(save = 'no', status = status)",
      "  writeLines('generation-validation', file.path(run_path, 'active-step'))",
      "  args <- c(",
      "    'validate-generation',",
      "    '--input', resolve(execution$outputs$upstream),",
      "    '--prompts', resolve(execution$inputs$prompts),",
      "    '--output', resolve(execution$outputs$portable),",
      "    '--fasta', resolve(execution$outputs$fasta),",
      "    '--validation', resolve(execution$outputs$validation),",
      "    '--num-tokens', as.character(parameters$max_new_tokens),",
      "    '--validate', parameters$validate,",
      "    if (isTRUE(parameters$return_probabilities)) '--return-probabilities'",
      "  )",
      ...
      "  unlink(file.path(dirname(normalizePath(value('--input'), mustWork = TRUE)), '..', 'active-step'))",
